...
describe('error-middleware', () => {
  ...
  `);
    ...
    test('mapped error, with debug and with log', async () => {
      const error = new Error('error');
      // eslint-disable-next-line functional/immutable-data
      error.stack = 'Error: error\nat Line1\nat Line2';
...
`);
      ...
      const [responseFactory, responseFactoryMocks] = useFunctionMock<ResponseFactory>([
        { parameters: [404], return: response },
      ]);
...
`);
      ...
      const errorMiddleware = createErrorMiddleware(responseFactory, true, logger, (e: unknown): HttpError => {
