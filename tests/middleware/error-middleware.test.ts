...
describe('createErrorMiddleware', () => {
  test('successful', async () => {
    ...
    const responseFactory = createFunctionMock<ResponseFactory>([]);
  ...
  test('error, without debug and without log', async () => {
    ...
    const responseBodyMocks: ObjectMocks<Response['body']> = [
      {
        name: 'end',
...
`);
    ...
    const responseBody = createObjectMock(responseBodyMocks);

    const responseMocks: ObjectMocks<Response> = [
      { name: 'body', value: responseBody },
      { name: 'headers', value: {} },
    ];

    const response = createObjectMock(responseMocks);

    const handlerMocks: FunctionMocks<Handler> = [{ parameters: [request], error }];

    const handler = createFunctionMock(handlerMocks);

    const responseFactoryMocks: FunctionMocks<ResponseFactory> = [{ parameters: [500], return: response }];

    const responseFactory = createFunctionMock(responseFactoryMocks);
...
`);
    ...
    const loggerMocks: ObjectMocks<Logger> = [
      {
        name: 'error',
...
`);
    ...
    const logger = createObjectMock(loggerMocks);
...
`);
    ...
    const handlerMocks: FunctionMocks<Handler> = [{ parameters: [request], error: error as unknown as Error }];
...
`);
    ...
    const handlerMocks: FunctionMocks<Handler> = [{ parameters: [request], error: httpError }];

    const handler = createFunctionMock(handlerMocks);

    const responseFactoryMocks: FunctionMocks<ResponseFactory> = [{ parameters: [400], return: response }];
